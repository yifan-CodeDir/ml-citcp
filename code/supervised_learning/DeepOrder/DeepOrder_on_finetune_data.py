...
if __name__ == '__main__':
    ...
    training_sample_list = [1998, 2010, 2008, 2005, 2017, 2020, 1999, 2044, 2017, 2011]
    ...
    for project in projects:
        ...
        # Reading the dataset
        df = pd.read_csv('Datasets/cross/proc_' + project + '_result.csv') ##############
        df.head()
        # values = pd.DataFrame()
        # values['Id'] =  range(0, 1)
        # values['env'] = 'bcel'  ################
        ...
        X_train, X_test, y_train, y_test = split_dataset(X, Y, 19833 + spilt)
        ...
        for index in range (1, Last_cycle+1):
            ...
            if (cycle_num <= Last_cycle and cycle_num in df['Cycle']):
                ...
                final_df_temp.to_csv('./result/finetune/' + project + '_detail.csv',
                                    header=True, mode='a',
                                    index=False)  ################
        ...
        output.to_csv('./result/finetune/'+ project +'.csv', mode='a', sep=";",header=True, index = False) ############
