...
if __name__ == '__main__':
    ...
    training_sample_list = [1998, 2010, 2008, 2005, 2017, 2020, 1999, 2044, 2017, 2065, 2011]
    ...
    for project in projects:
        ...
        # Reading the dataset
        df = pd.read_csv('Datasets/normal/proc_' + project + '_result.csv') ##############
        df.head()
        # values = pd.DataFrame()
        # values['Id'] =  range(0, 1)
        # values['env'] = 'bcel'  ################
        ...
        for index in range (1, Last_cycle+1):
            ...
            if (cycle_num <= Last_cycle and cycle_num in df['Cycle']):
                ...
                final_df_temp.to_csv('./result/normal/' + project + '_detail.csv',
                                    header=True, mode='a',
                                    index=False)  ################
        ...
        output.to_csv('./result/normal/'+ project +'.csv', mode='a', sep=";",header=True, index = False) ############
