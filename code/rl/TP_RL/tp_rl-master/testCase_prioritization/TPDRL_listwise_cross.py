...
#    -- fix end cycle issue
    ...
    for i in range(start_cycle, end_cycle - 1):
        ...
        if first_round:
            ...
            pre_model = "../experiments/cross/listwise/ppo1/listwise_PPO1_0_183.zip"
            # test_data_loader = TestCaseExecutionDataLoader('../data/new/nfe_result.csv', "enriched")
            # test_data = test_data_loader.load_data()
            # ci_cycle_logs = test_data_loader.pre_process()
            
            # # construct previous config
            # pre_conf = Config()
            # conf.train_data = args.train_data
            # conf.dataset_name = Path(args.train_data).stem
            # env = CIListWiseEnv(ci_cycle_logs[-1], pre_conf)
            # pre_env = Monitor(env, "../experiments/cross/listwise/ppo1/listwise_PPO1_0_183_monitor.csv")
            tp_agent = TPAgentUtil.load_model(algo=algo, env=env, path=pre_model)
