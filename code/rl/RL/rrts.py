...
if __name__ == '__main__':
    ...
    print('\nDATASET')
    # print(args + '\n')
    files = ['commons-bcel_result.csv', 'commons-configuration_result.csv', 'commons-csv_result.csv', 'commons-dbcp_result.csv', 'commons-text_result.csv', 'java-faker_result.csv', 'jedis_result.csv', 'jsoup_result.csv', 'maxwell_result.csv', 'nfe_result.csv']  
    # files = ['commons_codec_result.csv', 'commons_compress_result.csv', 'commons_imaging_result.csv', 'commons_io_result.csv', 'commons_lang_result.csv', 'commons_math_result.csv']  
    for file in files:
        ...
        data = data.rename(index=str,
                           columns={"A_priority": "A", "A_priority_with_time": "A_WITH_TIME", "B_priority": "B",
        ...
        for commit_id in commit_id_list:
            ...
            if agent.already_fitted_model:
                ...
                (action, action_p) = agent.get_action(
                    data_subset.drop(['test_class_name', 'cycle_id', 'current_failures', 'time'], axis='columns'))
                ...
                # print(pd.DataFrame(action_p, columns=2))
                ...
                score = agent.model.score(
                    data_subset.drop(['test_class_name', 'cycle_id', 'current_failures', 'time'], axis='columns'),
                    labels.loc[labels['cycle_id'] == commit_id][REWARD_SELECTOR])
                ...
                for j in range(0, len(data_subset)):
                    ...
                    cycle_id_array.append(data_subset.iloc[j]['cycle_id'])
                ...
                evaluation.insert(len(evaluation.columns), 'failures_percenteage', failures_percenteage_array,
                                  allow_duplicates=True)
                evaluation.insert(len(evaluation.columns), 'cycle_id', cycle_id_array, allow_duplicates=True)
                # print(data_subset.iloc[:,-5:])
                ...
                if len(evaluation[(evaluation['failures'] > 0)]) == 0:
                    evaluation.insert(len(evaluation.columns), 'failure_metric', [0] * len(evaluation),
                                      allow_duplicates=True)
                else:
                    failure_metric = 0
                    for row in evaluation[(evaluation['failures'] > 0)].itertuples():
                        failure_metric += (row[0] + 1) / len(evaluation)
                    failure_metric /= len(evaluation[(evaluation['failures'] > 0)])
                    failure_metric_list.append(failure_metric)
                    evaluation.insert(len(evaluation.columns), 'failure_metric', [failure_metric] * len(evaluation),
                                      allow_duplicates=True)
                    evaluation.to_csv('4class_experiments/1_layer_detail/' + file.replace('_result', '_failure_detail'),
                                      header=True, mode='a',
                                      index=False)  ################

                evaluation.to_csv('4class_experiments/1_layer_detail/' + file.replace('_result', '_whole_detail'),
                                  header=True, mode='a',
                                  index=False)  ################
                print(estimated_fpa)

                # metriche per ciclo
                output_data_temp = pd.DataFrame({"cycle_id": [commit_id], "num_testsuite": [len(data_subset)],
                                                 "NORMALIZED_FPA": [estimated_fpa / optimal_fpa], "accuracy": [score],
                                                 "total_failures_in_cycle": [evaluation['failures'].sum()],
                                                 "exec_time": [evaluation['time'].sum()],
                                                 "optimal_failures_25%": [optimal_failures_25],
                                                 "failures_in_25%_ordered": [failures_in_25_ordered],
                                                 "optimal_exec_time_25%": [optimal_exec_time_25],
                                                 "exec_time_25%": [exec_time_25],
                                                 "optimal_failures_50%": [optimal_failures_50],
                                                 "failures_in_50%_ordered": [failures_in_50_ordered],
                                                 "optimal_exec_time_50%": [optimal_exec_time_50],
                                                 "exec_time_50%": [exec_time_50],
                                                 "optimal_failures_75%": [optimal_failures_75],
                                                 "failures_in_75%_ordered": [failures_in_75_ordered],
                                                 "optimal_exec_time_75%": [optimal_exec_time_75],
            ...
            agent.model_fitting(
                batch_dataset.drop(['test_class_name', 'cycle_id', 'time'], axis='columns').iloc[:, :-1],
                batch_dataset.drop(['test_class_name', 'cycle_id', 'time'], axis='columns').iloc[:, -1])
        ...
        output_data.insert(len(output_data.columns), 'failure_metric', [fm]*len(output_data), allow_duplicates=True)
        if not os.path.exists('4class_experiments/1_layer_detail'):
            os.makedirs('4class_experiments/1_layer_detail')

        if not os.path.isfile('4class_experiments/1_layer_detail/' + file.replace('_result', '_summary')):
            output_data.to_csv('4class_experiments/1_layer_detail/' + file.replace('_result', '_summary'), index=False,
                               header=True)
        ...
        # if not os.path.isfile('history_sensitivity_analysis/4_layer/' + str(BATCH_SIZE) + args.replace('_result_data', '_summary')):
        #    output_data.to_csv('history_sensitivity_analysis/4_layer/' + str(BATCH_SIZE) + args.replace('_result_data', '_summary'), index = False, header = True)
        # else: # else it exists so append without writing the header
        #    output_data.to_csv('history_sensitivity_analysis/4_layer/' + str(BATCH_SIZE) + args.replace('_result_data', '_summary'),index = False, mode = 'a', header = False)
