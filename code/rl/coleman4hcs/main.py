...
DEFAULT_SCHED_TIME_RATIO = [1]

# example command: python main.py --project_dir 'COLEMAN_dataset'  --policies  'FRR' --output_dir 'results/normal' --datasets 'commons-bcel' --rewards TimeRankReward
...
def run_experiments_with_threads(repo_path, dataset, policies, window_sizes=DEFAULT_WINDOW_SIZES,
    ...
    if considers_variants:
        ...
        scenario = scenarios.IndustrialDatasetScenarioProvider(
            f"{repo_path}/proc_{dataset}_result.csv",
            sched_time_ratio)
