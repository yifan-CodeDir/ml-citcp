...
for project in projects:
    f = pd.read_csv(project + "_summary.csv")
    training_dict[project] = np.mean(f['learning_time'].values) # (in seconds)
...
for project in projects:
    ...
    testing_dict[project] = np.mean(f.iloc[start_cycle:]['prediction_time'].values)   # (in seconds)
